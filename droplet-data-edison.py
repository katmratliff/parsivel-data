# -*- coding: utf-8 -*-
"""
PURPOSE: - import raw OTT Parsivel disdrometer data from EDISON parsivel
...
MIS_cols = ["date", "time", "intensity", "total_precip", "WC_WaWa",
            "WC_MS", "WC_NWS", "reflectivity", "visibility", "num_particles",
            "temp", "current", "voltage", "KE", "spectrum",
            *list(range(0, 1024))]
df = pd.read_csv(FILENAME, names=MIS_cols, index_col=False, engine='python')
# delete rows where spectral data doesn't exist
df = df.drop(df[df.iloc[:, 14] != '<SPECTRUM>'].index)
# delete first rows if specified
if del_rows:
    drop_data = df.iloc[del_rows:, :14]
    df_spec_raw = df.iloc[del_rows:, 14:1038]
else:
    drop_data = df.iloc[:, :14]
    df_spec_raw = df.iloc[:, 14:1038]
...
if zscore_tol:
    drop_data = drop_data[~(np.abs(drop_data.num_particles.astype(float) -
                drop_data.num_particles.astype(float).mean())
                >(zscore_tol * drop_data.num_particles.astype(float).std()))]

# stats from parsivel
intensity_avg_mmhr = np.mean(drop_data.intensity.astype(float))
intensity_std_mmhr = np.std(drop_data.intensity.astype(float), ddof=1)

intensity_avg_inhr = np.mean(drop_data.intensity.astype(float) * mm_to_in) 
intensity_std_inhr = np.std(drop_data.intensity.astype(float) * mm_to_in, ddof=1)

num_particles_avg = np.mean(drop_data.num_particles.astype(float))
num_particles_std = np.std(drop_data.num_particles.astype(float), ddof=1)

kinetic_energy_avg = np.mean(drop_data.KE.astype(float))
kinetic_energy_std = np.std(drop_data.KE.astype(float), ddof=1)
